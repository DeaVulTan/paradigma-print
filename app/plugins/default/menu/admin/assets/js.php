    ...
    $(document).ready(function()
{
    ...
    function add_item(type){ 
        ...
        if(type=='page') 
            var options = $('#new-'+type+' .item-options').val(); 
        else 
            options = '';
        if($('#new-'+type+' .-img').parent().attr('aria-checked') == 'true'){
            var icontype='img'; 
            var icon =   $('#new-'+type+' #img-input input[name=item-icon]').val();
        }
        else if($('#new-'+type+' .-css').parent().attr('aria-checked') == 'true') { 
            var icontype='css'; 
            var icon   =   $('#new-'+type+' #css-input input[name=item-icon]').val();
        }
        ...
        if(name=='' || name.length>75 || (type == 'url' && ValidURL(call)==false)){
            ...
            if(id ==    ""){
            var id  =   rand();
            t   =   "<li class='dd-item' id='"+id+"' data-id='"+id+"'><div class='manage-item'> <a class='btn btn-success btn-xs public_btn' onclick=\"publicid('"+id+"'); return;\"><i id='icon-"+id+"' class='fa fa-check-square'></i></a><a class='btn btn-info btn-xs' id='edit-"+id+"' data-toggle='modal' data-target='#new-"+type+"' onclick=\"edit_item('"+id+"','"+name+"','"+type+"','"+call+"','"+options+"', '"+icontype+"','"+icon+"','"+desc+"','"+color+"'); \"><i class='fa fa-pencil-square-o'></i></a><a class='btn btn-danger btn-xs' onclick=\"deleteid('"+id+"'); return;\"><i class='fa fa-times-circle'></i></a> </div><div class='dd-handle'>"+name+"</div></li>";
