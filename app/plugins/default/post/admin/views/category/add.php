...
<div class="row" style="margin-bottom: 20px; margin-top: -15px;">
    <div class="col-sm-12">
        <h3>
            ...
            <?php } ?>
            <?php echo __('Category','post'); ?>  <small><?php echo __('Add','post'); ?></small>
        </h3>
    </div>
</div>
<div class="row">
    <div class="col-sm-12">
        <form name="frm_Category_New" id="frm_Category_New"
            class="form-horizontal" role="form"
            action="<?php echo admin_url(); ?>" method="post"
            ...
            <div
                ...
                <div class="col-sm-10">
                    <select name="category_parent" class="form-control">
                        ...
                        ?>
                                    <option value="<?php echo $items['id']; ?>"<?php echo $this->controller->post->id == $items['id'] ? 'selected="selected"' : '' ?>><?php echo $items['category_name']; ?></option>
                        <?php } }?>
                    </select>
...
function Category_New(){
	...
	$.post($('#frm_Category_New').attr('action'),$('#frm_Category_New').serialize(),function(obj){
		...
		if (obj.error == 1){
			...
			$.notification({type:"success",img:"",width:"400",content:"<i class='fa fa-check fa-2x'></i> <?php echo __('Category is added successfully','post'); ?>",html:true,autoClose:true,timeOut:"1500",delay:"0",position:"topRight",effect:"fade",animate:"fadeUp",easing:"jswing",onStart:function(id){ console.log(' onStart : notification id = '+id); },onShow:function(id){ console.log(' onShow : notification id = '+id); },onClose:function(id){ console.log(' onClose : notification id = '+id); },duration:"300"});
