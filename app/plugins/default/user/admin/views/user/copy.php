...
<div class="row" style="margin-bottom: 20px;margin-top:-15px;">
    <div class="col-sm-12">
        <h3>
            ...
            <?php } ?>
            <?php echo __('User','user'); ?>  <small><?php echo __('Copy','user'); ?></small>
        </h3>
    </div>
</div>
<div class="row">
    <div class="col-sm-12">
        <form name="frm_User_Copy" id="frm_User_Copy" class="form-horizontal" role="form" action="<?php echo admin_url('token='.$this->token); ?>" method="post" onsubmit="return false;">
           ...
           <div class="form-group <?php $this->error_class("user_activation");?>">
            ...
            <?php echo Pf::event()->trigger("filter","user-copy-form"); ?>
        </form>
    </div>
</div>
<script>
...
function User_Copy(){
	...
	$.post($('#frm_User_Copy').attr('action'),$('#frm_User_Copy').serialize(),function(obj){
		$("#main-content").unmask();
		if (obj.error == 1){
			...
			$.notification({type:"success",img:"",width:"400",content:"<i class='fa fa-check fa-2x'></i> <?php echo __('User is copied successfully','user'); ?>",html:true,autoClose:true,timeOut:"1500",delay:"0",position:"topRight",effect:"fade",animate:"fadeUp",easing:"jswing",onStart:function(id){ console.log(' onStart : notification id = '+id); },onShow:function(id){ console.log(' onShow : notification id = '+id); },onClose:function(id){ console.log(' onClose : notification id = '+id); },duration:"300"});
