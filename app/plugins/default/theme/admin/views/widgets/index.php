    ...
    <div class="col-md-8">
        <div class="btn-group" id="bulk-action-container"
            ...
            <button type="button"
                ...
                <?php echo __('Bulk Actions','theme'); ?> <span
                    class="caret"></span>
            </button>
            <ul class="dropdown-menu" role="menu">
                <li><a href="#" onclick="bulk_action('activate'); return false;"><?php echo __('Activate','theme'); ?></a></li>
                <li><a href="#" onclick="bulk_action('deactivate'); return false;"><?php echo __('Deactivate','theme'); ?></a></li>
            </ul>
        </div>
    </div>
    ...
    <div class="col-md-6">
        <div class="btn-group">
            <button type="button"
                ...
                data-toggle="dropdown">
                <span id="view-title"><?php echo __('All', 'theme'); ?></span>
                <span class="caret"></span>
            </button>
            <ul class="dropdown-menu" role="menu">
                <li><a
                    href="<?php echo admin_url("admin-page=themes&sub_page=widgets", false); ?>"> <?php echo __('All', 'theme'); ?></a></li>
                <li><a href="javascript:view_allstatus('activate');"> <?php echo __('Activate', 'theme'); ?></a></li>
                <li><a href="javascript:view_allstatus('deactivate');"> <?php echo __('Deactivate', 'theme'); ?></a></li>
            </ul>
        </div>
    </div>
    <div class="col-md-6">
                        ...
                        <thead>
                            <tr>
                                ...
                                <th style="background: #3c8dbc; color: white;width: 250px;">
                                    ...
                                    ?>
                                    <div style="cursor: pointer; position: relative;" id="order_title" onclick="widgets_Oder(this,'name');">
                                        <?php echo __("Widget Name","theme"); ?> 
                                ...
                                <th style="background: #3c8dbc; color: white;width:100px;"><?php echo __('Version','theme'); ?></th>
                                <th style="background: #3c8dbc; color: white;width:858px"><?php echo __('Description','theme'); ?></th>
                                <th style="background: #3c8dbc; color: white;width:88px;"><?php echo __("Actions","theme"); ?> </th>
                            </tr>
                        </thead>
                        <tbody>
                    ...
                    <?php
                        ...
                        foreach ( $this->records as $index => $record ) {
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php if (!in_array($index, $this->actived_widgets)){?>
                        			<span id="actions_<?php echo $stt;?>"><a class="btn btn-primary btn-xs" href="javascript:change_actions('<?php echo $index;?>','activate',<?php echo $stt;?>);"><?php echo __('Activate', 'theme'); ?></a></span>
                        			<?php }else{ ?>
                        			<span id="actions_<?php echo $stt;?>"><a class="btn btn-danger btn-xs" href="javascript:change_actions('<?php echo $index;?>','deactivate',<?php echo $stt;?>);"><?php echo __('Deactivate', 'theme'); ?></a></span>
                        			<?php }?>
                                </td>
                            </tr>
                    <?php $stt++;}?>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><?php echo __('Widgets','theme'); ?> <?php echo __('Search','theme'); ?> </h4>
            </div>
            ...
            <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="subject" class="col-sm-2 control-label">
                            <?php echo __("Actions","theme"); ?>
                        </label>
                        <div class="col-sm-10">
                            <div class="row"><?php foreach($this->controller->widgets_model->elements_value["action"] as $k => $v){ ?>    <label class="checkbox-inline"><?php echo form_checkbox("search[action][]",$k,(isset($this->controller->get->search["action"]) && in_array($k,$this->controller->get->search["action"]))?true:false);?> <?php echo __($v,'theme'); ?></label><?php }?></div>
                        </div>
                    </div>
...
function bulk_action(type){
	if (type != undefined){
		$.sModal({
	    	...
	    	content:'<?php echo __('Are you sure to perform this action?','theme'); ?>',
	    	animate:'fadeDown',
	    	buttons:[
	                 {
                        	    ...
                        	    $.post($('#frm-bulk-action').attr('action')+'&type='+type,$('#frm-bulk-action').serialize(),function(obj){
    	              				...
    	              				$('#main-content input[type="radio"]').iCheck({radioClass: 'iradio_minimal'});
    	              				 ...
    	              				 if (obj.error == 0){
    	              				   switch(obj.action){
    	              					    case 'activate':
        	              				        $.notification({type:"success",img:"",width:"400",content:"<i class='fa fa-check fa-2x'></i> <?php echo __('Widgets is activated successfully','theme'); ?>",html:true,autoClose:true,timeOut:"1500",delay:"0",position:"topRight",effect:"fade",animate:"fadeUp",easing:"jswing",onStart:function(id){ console.log(' onStart : notification id = '+id); },onShow:function(id){ console.log(' onShow : notification id = '+id); },onClose:function(id){ console.log(' onClose : notification id = '+id); },duration:"300"});
        	              				    break;
        	              				    case 'deactivate':
        	              				        $.notification({type:"success",img:"",width:"400",content:"<i class='fa fa-check fa-2x'></i> <?php echo __('Widgets is deactivated successfully','theme'); ?>",html:true,autoClose:true,timeOut:"1500",delay:"0",position:"topRight",effect:"fade",animate:"fadeUp",easing:"jswing",onStart:function(id){ console.log(' onStart : notification id = '+id); },onShow:function(id){ console.log(' onShow : notification id = '+id); },onClose:function(id){ console.log(' onClose : notification id = '+id); },duration:"300"});
    										break;
    	              				   }
    	              				}else{
...
function cancel_widgets_Search(){
	$("#main-content").mask("<?php echo __('Loading...','theme'); ?>");
	$('#main-content').load('<?php echo admin_url('&sub_page=widgets&search='); ?>',function(){
		$("#main-content").unmask();
		$('#main-content input[type="checkbox"]').iCheck({checkboxClass: 'icheckbox_minimal'});
		$('#main-content input[type="radio"]').iCheck({radioClass: 'iradio_minimal'});
	});
...
function change_actions(id,status,stt) {
    $("#main-content").mask("<?php echo __('Loading...','theme'); ?>");
    $.post("<?php echo admin_url($this->action.'=change_actions');?>", {action: "update-status",status: status,id: id},
    	function (data) {
    	$("#main-content").unmask();
        var result = "";
        if (status == "deactivate") {
            result = "<a class='btn btn-primary btn-xs' href=\"javascript:change_actions('" + id + "','activate',"+stt+")\"><?php echo __("Activate","theme"); ?></a>";
        } else {
        	result = "<a class='btn btn-danger btn-xs' href=\"javascript:change_actions('" + id + "','deactivate',"+stt+")\"><?php echo __("Deactivate","theme"); ?></a>";
        }
...
function view_allstatus(status) {
    $("#main-content").mask("<?php echo __('Loading...','theme'); ?>");
    $.post("<?php echo admin_url('&sub_page=widgets'); ?>", {action: "view-status",status: status},
    	function (html) {
    	$("#main-content").unmask();
    	$('#main-content').html(html);
    	if(status == 'activate'){
