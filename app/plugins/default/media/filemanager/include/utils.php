...
function create_img_gd($imgfile, $imgthumb, $newwidth, $newheight = null)
{
    if (image_check_memory_usage($imgfile, $newwidth, $newheight)) {
        require_once('simple_image.php');
        $img = new SimpleImage($imgfile);
        $img->thumbnail($newwidth, $newheight)->save($imgthumb, 90);
        return true;
    }else{
        return false;
    }
...
function create_img($imgfile, $imgthumb, $newwidth, $newheight = null)
{
    if (image_check_memory_usage($imgfile, $newwidth, $newheight)) {
        require_once('simple_image.php');
        $img = new SimpleImage($imgfile);
        $img->thumbnail($newwidth, $newheight)->save($imgthumb, 80);
        return true;
    } else {
        return false;
    }
