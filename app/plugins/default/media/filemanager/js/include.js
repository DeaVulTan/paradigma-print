...
$(document).ready(function() {
    ...
    $('ul.grid').on('click', '.delete-file', function() {
        ...
        bootbox.confirm(_this.attr('data-confirm'), $('#cancel').val(), $('#ok').val(), function(result) {
            if (result == true) {
                execute_action('delete_file', _this.attr('data-path'), '', '', '', '', _this.attr('data-token'));
    ...
    $('ul.grid').on('click', '.delete-folder', function() {
        ...
        bootbox.confirm(_this.attr('data-confirm'), $('#cancel').val(), $('#ok').val(), function(result) {
            if (result === true) {
                var result = execute_action('delete_folder', _this.attr('data-path'), '', '', '', '', _this.attr('data-token'));
                console.log(result);
                if (result === true) {
...
function apply(file, external) {
    ...
    var path = $('#cur_dir_full').val();
    //path = path.replace('\\', '/');
    var base_url = $('#base_url_true').val();
...
function execute_action(action, file1, file2, name, container, function_name, token) {
    ...
    if (name !== null) {
        name = fix_filename(name);
        var result = false;
        $.post("execute.php?action=" + action, {path: file1, path_thumb: file2, name: name.replace('/', ''), token: token}, function(msg) {
            if (msg !== '') {
                bootbox.alert(msg);
                return false;
            }
