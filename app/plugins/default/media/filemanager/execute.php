...
$path_name = isset($_POST['path']) ? $_POST['path'] : '';
$path_tmp = $current_path . 'thumbnails/' . $_POST['path'];
...
if (isset($_GET['action'])) {
    switch ($_GET['action']) {
        case 'delete_file':
            if ($delete_files) {
                if (!Pf_Plugin_CSRF::is_valid($token, $path_name)) {
                    die('Token incorrect');
                }
                unlink($path);
        ...
        case 'create_folder':
            if ($create_folders) {
                create_folder(fix_path($path, $transliteration, $convert_spaces), fix_path($path_tmp, $transliteration, $convert_spaces));
            }
            break;
        case 'rename_folder':
            if ($rename_folders) {
                ...
                if (!empty($name)) {
                    ...
                    rename_folder($path_tmp, $name, $transliteration);
                    if ($fixed_image_creation) {
                        foreach ($fixed_path_from_filemanager as $k => $paths) {
                            if ($paths != "" && $paths[strlen($paths) - 1] != "/")
                                $paths.="/";
        ...
        case 'create_file':
            ...
            $path_tmp = str_replace($old_name, $name, $path_tmp);
        ...
        case 'rename_file':
            if ($rename_files) {
                ...
                if (!empty($name)) {
                    ...
                    rename_file($path_tmp, $name, $transliteration);
        ...
        case 'paste_clipboard':
            ...
            if (is_really_writable($path) === FALSE || is_really_writable($path_tmp) === FALSE) {
                die($path . '--' . $path_tmp . '--' . lang_Dir_No_Write);
            }
            ...
            if ($action == 'copy') {
                ...
                rcopy($data['path_thumb'], $path_tmp);
            } elseif ($action == 'cut') {
                rrename($data['path'], $path);
                rrename($data['path_thumb'], $path_tmp);
