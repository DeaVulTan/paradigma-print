...
//remember last position
    ...
    if (trim($rfm_subfolder) == "") {
        $cur_dir = $dir_apply_img . $subdir;
        $cur_dir_tmp = $dir_apply_img_tmp . $subdir;
        $cur_dir_full = $upload_dir . $subdir;
        $cur_path = $current_path . $subdir;
        $thumbs_path = ABSPATH . '/' . $thumbs_base_path;
        $thumbs_path_show = $thumbs_base_path;
        $parent = $subdir;
    } else {
        $cur_dir = $dir_apply_img . $rfm_subfolder . $subdir;
        $cur_dir_tmp = $dir_apply_img_tmp . $rfm_subfolder . $subdir;
        $cur_dir_full = $upload_dir . $subdir;
        $cur_path = $current_path . $rfm_subfolder . $subdir;
        $thumbs_path = ABSPATH . '/' . $thumbs_base_path . $rfm_subfolder;
        $thumbs_path_show = $thumbs_base_path . $rfm_subfolder;
...
// language
    ...
    <html xmlns="https://www.w3.org/1999/xhtml">
        ...
        <body>
            ...
            <input type="hidden" id="cur_dir_full" value="<?php echo $cur_dir_full; ?>" />
...
//php sorting
    ...
    <?php if (@opendir($current_path . $rfm_subfolder . $subdir) === FALSE) { ?>
                        ...
                        <ul class="grid cs-style-2 <?php echo "list-view" . $view; ?>" id="main-item-container">
                            ...
                            if ($file == "..") {
                                ...
                                <?php } else { ?>
                                    ...
                                    <?php if ($show_folder_size) { ?><div class="file-size"><?php echo makeSize($file_array['size']) ?></div><?php } ?>
                                            ...
                                            <figcaption>
                                                <a href="javascript:void('')" class="tip-left edit-button rename-file-paths <?php if ($rename_folders && !$file_prevent_rename) echo "rename-folder"; ?>" title="<?php echo lang_Rename ?>" data-path="<?php echo $rfm_subfolder . $subdir . $file; ?>" data-thumb="">
                                                    <i class="icon-pencil <?php if (!$rename_folders || $file_prevent_rename) echo 'icon-white'; ?>"></i></a>
                                                <a href="javascript:void('')" class="tip-left erase-button <?php if ($delete_folders && !$file_prevent_delete) echo "delete-folder"; ?>" title="<?php echo lang_Erase ?>" data-confirm="<?php echo lang_Confirm_Folder_del; ?>" data-path="<?php echo $rfm_subfolder . $subdir . $file; ?>"  data-thumb="" data-token="<?php echo Pf_Plugin_CSRF::token($rfm_subfolder . $subdir . $file); ?>">
                            ...
                            foreach ($files as $nu => $file_array) {
                                ...
                                if (in_array($extension_lower, $ext_img)) {
                                    $src = $base_url . MEDIA_UPLOAD_DIR . $cur_dir_tmp . rawurlencode($file);
                                    $mini_src = $src_thumb = $thumbs_path.$subdir. $file;
                                    $mini_src_show = $src_thumb_show = BASE_URL . $thumbs_base_path . $subdir . $file;
                                    //add in thumbs folder if not exist
                                            ...
                                            <a href="javascript:void('')" class="link" data-file="<?php echo $file; ?>" data-field_id="<?php echo $field_id; ?>" data-function="<?php echo $apply; ?>">
                                                <div class="img-precontainer">
                                                    ...
                                                    <div class="img-container">
                                                        <span></span>
                                                        <img alt="<?php echo $filename . " thumbnails"; ?>" class="<?php echo $show_original ? "original" : "" ?> <?php echo $is_icon_thumb ? "icon" : "" ?>" src="<?php echo $src_thumb_show; ?>">
                                                    </div>
                                                </div>
                                                <div class="img-precontainer-mini <?php if ($is_img) echo 'original-thumb' ?>">
                                                    ...
                                                    <div class="img-container-mini">
                                                        <span></span>
                <?php if ($mini_src != "") { ?>
                                                            <img alt="<?php echo $filename . " thumbnails"; ?>" class="<?php echo $show_original_mini ? "original" : "" ?> <?php echo $is_icon_thumb_mini ? "icon" : "" ?>" src="<?php echo $mini_src_show; ?>">
                <?php } ?>
                                                    </div>
                                                </div>
                ...
                <?php } ?>
                                                    <a href="javascript:void('')" class="tip-left edit-button rename-file-paths <?php if ($rename_files && !$file_prevent_rename) echo "rename-file"; ?>" title="<?php echo lang_Rename ?>" data-path="<?php echo $rfm_subfolder . $subdir . $file; ?>" data-thumb="" data-token="<?php echo Pf_Plugin_CSRF::token(); ?>">
                                                        <i class="icon-pencil <?php if (!$rename_files || $file_prevent_rename) echo 'icon-white'; ?>"></i></a>

                                                    <a href="javascript:void('')" class="tip-left erase-button <?php if ($delete_files && !$file_prevent_delete) echo "delete-file"; ?>" title="<?php echo lang_Erase ?>" data-confirm="<?php echo lang_Confirm_del; ?>" data-path="<?php echo $rfm_subfolder . $subdir . $file; ?>" data-thumb="" data-token="<?php echo Pf_Plugin_CSRF::token($rfm_subfolder . $subdir . $file); ?>">
