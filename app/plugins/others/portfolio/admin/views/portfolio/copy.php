...
<div class="row" style="margin-bottom: 20px;margin-top:-15px;">
    <div class="col-sm-12">
        <h3>
            ...
            <?php } ?>
            <?php echo __('Portfolios','portfolio'); ?>  <small><?php echo __('Copy','portfolio'); ?></small>
        </h3>
    </div>
</div>
<div class="row">
    <div class="col-sm-12">
        <form name="frm_Portfolio_Copy" id="frm_Portfolio_Copy" class="form-horizontal" role="form" action="<?php echo admin_url('token='.$this->token); ?>" method="post" onsubmit="return false;">
                               ...
                               <?php if(count($images)):?>
                                    <?php foreach($images as $key=>$rows):?>
                                        <div class="thumbview-gal thumbview">
                                            <div class="inner">
                                                   <span class="control">
                                                      <a href="<?php echo RELATIVE_PATH . "/".ADMIN_FOLDER."/plugins/media/filemanager/dialog.php?type=1&field_id=tmp_".$key; ?>"
            ...
            <?php echo Pf::event()->trigger("filter","portfolio-copy-form"); ?>
        </form>
    </div>
</div>
...
function Portfolio_Copy(){
	$("#main-content").mask("<?php echo __('Loading...','portfolio'); ?>");
	try{
       tinymce.triggerSave(); 
        tinymce.remove();
    }catch(e){}
	$.post($('#frm_Portfolio_Copy').attr('action'),$('#frm_Portfolio_Copy').serialize(),function(obj){
		$("#main-content").unmask();
		if (obj.error == 1){
