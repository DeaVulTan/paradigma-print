...
<?php 
   $touser =  $this->controller->post->touser; 
   if(isset($touser) && $touser != NULL){
       foreach($touser as $key => $value){
           if($value == NULL){
               unset($key);
           }else{
               $arr_touser[$key] = $value;
           }
       }
   }
?>
<div class="row" style="margin-bottom: 20px;margin-top:-15px;">
    <div class="col-sm-12">
        <h3>
            ...
            <?php } ?>
            <?php echo __('Announcement','announcement'); ?>     <small><?php echo __('Copy','announcement'); ?></small>
        </h3>
    </div>
</div>
<div class="row">
    <div class="col-sm-12">
        <form name="frm_Announcement_Copy" id="frm_Announcement_Copy" class="form-horizontal" role="form" action="<?php echo admin_url('token='.$this->token); ?>" method="post" onsubmit="return false;">
            ...
            <div class="form-group <?php $this->error_class("announcement_to");?> col-sm-6 pull-right">
                ...
                <div class="col-sm-8" id="test">
                    <input  id="announcement_touser" type = "hidden" value= "<?php if(isset($arr_touser)){echo implode(",", $arr_touser);}?>" name="announcement_touser" >
                    <div id="ms-right">
                    </div>
                    <script type="text/javascript">
                      $('#announcement_touser').attr('type','hidden');
                        ...
                        var ms = $('#ms-right').magicSuggest({
                          value: [<?php if(isset($arr_touser)){foreach ($arr_touser as $user){echo '"'.$user.'",';}}?>],
                          valueField: 'name',
            ...
            <div class="form-group <?php $this->error_class("announcement_to");?>">
                <label for="subject" class="col-sm-2 control-label">
                   ...
                   <?php 
                       $togroup= $this->controller->post->togroup;
                       foreach ($togroup as $role) {
                          $true[$role] = true;
                       }
                   ?>
                </label>
                <div class="col-sm-10" id = "togr">
                    <?php
                        $true[100]= !empty($true[100])?"checked":FALSE;
                        echo '<label class="checkbox-inline" style="padding-left:0px;">';
                            echo form_checkbox('togroup[]',100,' class="all"').__('All','announcement');
                        echo '</label>';
                    ?>
                    <?php
                    ...
                    <?php $this->error_message("announcement_status")?>
                     <script type="text/javascript">
                         $("#togr").on('ifChecked', function(event) {
                             $("input[type='checkbox'].one", "#togr").iCheck("disable");
                             $("#togroup").iCheck('enable');
                         });
                         $("#togr").on('ifUnchecked', function(event) {
                             $("input[type='checkbox'].one", "#togr").iCheck("enable");
                         });
                    </script>
                </div>
            </div>
            ...
            <?php echo Pf::event()->trigger("filter","announcement-copy-form"); ?>
        </form>
    </div>
</div>
<script>
...
function Announcement_Copy(){
	$("#main-content").mask("<?php echo __('Loading...','announcement'); ?>");
	try{
       tinymce.triggerSave(); 
        tinymce.remove();
    }catch(e){}
	$.post($('#frm_Announcement_Copy').attr('action'),$('#frm_Announcement_Copy').serialize(),function(obj){
		$("#main-content").unmask();
		if (obj.error == 1){
			...
			$.notification({type:"success",img:"",width:"400",content:"<i class='fa fa-check fa-2x'></i> <?php echo __('Announcement is copied successfully','announcement'); ?>",html:true,autoClose:true,timeOut:"1500",delay:"0",position:"topRight",effect:"fade",animate:"fadeUp",easing:"jswing",onStart:function(id){ console.log(' onStart : notification id = '+id); },onShow:function(id){ console.log(' onShow : notification id = '+id); },onClose:function(id){ console.log(' onClose : notification id = '+id); },duration:"300"});
