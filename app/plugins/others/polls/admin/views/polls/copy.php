...
<div class="row" style="margin-bottom: 20px;margin-top:-15px;">
    <div class="col-sm-12">
        <h3>
            ...
            <?php } ?>
            Polls  <small><?php echo __('Copy','polls'); ?></small>
        </h3>
    </div>
</div>
<div class="row">
    <div class="col-sm-12">
        <form name="frm_Polls_Copy" id="frm_Polls_Copy" class="form-horizontal" role="form" action="<?php echo admin_url('token='.$this->token); ?>" method="post" onsubmit="return false;">
            ...
            <div id="ansewer" >
               <?php if (!empty($this->controller->post->answers) && is_array($this->controller->post->answers)) {
                    foreach ($this->controller->post->answers as $field) {
                        if(!(empty($field['pollsa_answers']))) {
                           ...
                           <div class="form-group custom-cols">
                                ...
                                <div class="col-sm-10" style="padding:0px">
                                    <div class="col-sm-11"><input type="text" name='answer[]' value="<?php echo $field['pollsa_answers']; ?>" class="form-control"  /></div>
            ...
            <div class="form-group" id="addMore">
                ...
                <div class="col-sm-10">
                    <div class="form-inline">
                        <button type="button" id='btnAddField' class="btn btn-info"><?php echo __('Add Answer', 'polls'); ?></button>
                    </div>
                </div>
            </div>
             <div class="form-group" id="addMore">
            ...
            <?php echo Pf::event()->trigger("filter","polls-copy-form"); ?>
        </form>
    </div>
</div>
<script>
...
function Polls_Copy(){
	...
	$.post($('#frm_Polls_Copy').attr('action'),$('#frm_Polls_Copy').serialize(),function(obj){
		$("#main-content").unmask();
		if (obj.error == 1){
			...
			$.notification({type:"success",img:"",width:"400",content:"<i class='fa fa-check fa-2x'></i> <?php echo __('Poll is copied successfully','polls'); ?>",html:true,autoClose:true,timeOut:"1500",delay:"0",position:"topRight",effect:"fade",animate:"fadeUp",easing:"jswing",onStart:function(id){ console.log(' onStart : notification id = '+id); },onShow:function(id){ console.log(' onShow : notification id = '+id); },onClose:function(id){ console.log(' onClose : notification id = '+id); },duration:"300"});
