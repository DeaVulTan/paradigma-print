...
<div class="row" style="margin-bottom: 20px;margin-top:-15px;">
    <div class="col-sm-12">
        <h3>
            ...
            <?php } ?>
            <?php echo __('Popup','popup'); ?>  <small><?php echo __('Add','popup'); ?></small>
        </h3>
    </div>
</div>
<div class="row">
    <div class="col-sm-12">
        <form name="frm_Popup_New" id="frm_Popup_New" class="form-horizontal" role="form" action="<?php echo admin_url(); ?>" method="post" onsubmit="return false;">
            ...
            <?php echo Pf::event()->trigger("filter","popup-adding-form"); ?>
        </form>
    </div>
</div>
<script>
...
function Popup_New(){
	...
	$.post($('#frm_Popup_New').attr('action'),$('#frm_Popup_New').serialize(),function(obj){
		$("#main-content").unmask();
		if (obj.error == 1){
			...
			$.notification({type:"success",img:"",width:"400",content:"<i class='fa fa-check fa-2x'></i> <?php echo __('Item is added successfully','popup'); ?>",html:true,autoClose:true,timeOut:"1500",delay:"0",position:"topRight",effect:"fade",animate:"fadeUp",easing:"jswing",onStart:function(id){ console.log(' onStart : notification id = '+id); },onShow:function(id){ console.log(' onShow : notification id = '+id); },onClose:function(id){ console.log(' onClose : notification id = '+id); },duration:"300"});
