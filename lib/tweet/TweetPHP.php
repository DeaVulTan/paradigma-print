<?php
 ...
 class TweetPHP {
    ...
    public function  __construct ($options = array()) {
      ...
      array(
          'consumer_key'          => 'RkES9SV3y19Q9OCPJo13mW86R',
          'consumer_secret'       => '2FawYCF4Bgox76AdiXJtnpnozdQXxPYXwwk8PXSYLSPE5mPIKS',
          'access_token'          => '2868601663-Wmul9WYrdN8vVCS7eMZ2IlItmeFTagV5IHMVfjG',
          'access_token_secret'   => 'Y02i3uClS9ML5Ncq6Xwx8uwTF7Ci6nWmIh6mfFDsx9XAp',
          'twitter_screen_name'   => '',
          'cache_file'            => '', // Where on the server to save the cached formatted tweets
          'cache_file_raw'        => '', // Where on the server to save the cached raw tweets
          'cachetime'             => 1, // Seconds to cache feed (1 hour).
          'tweets_to_display'     => '', // How many tweets to fetch
    ...
    private function fetch_tweets () {
      ...
      if ($response_code == 200) {
        ...
        $file = fopen($this->options['cache_file'], 'a');
        fwrite($file, $html); 
        fclose($file);

        // Save the raw data array to a file. 
        $file = fopen($this->options['cache_file_raw'], 'a');
